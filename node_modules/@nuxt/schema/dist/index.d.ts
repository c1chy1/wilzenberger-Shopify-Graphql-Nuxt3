...
declare type RouterOptions = Partial<Omit<RouterOptions$1, 'history' | 'routes'>> & {
    history?: (baseURL?: string) => RouterHistory;
    routes?: (_routes: RouterOptions$1['routes']) => RouterOptions$1['routes'];
    hashMode?: boolean;
};
declare type RouterConfig = RouterOptions;
/**
 * Only JSON serializable router options are configurable from nuxt config
 */
declare type RouterConfigSerializable = Pick<RouterConfig, 'linkActiveClass' | 'linkExactActiveClass' | 'end' | 'sensitive' | 'strict' | 'hashMode'>;
...
declare type _TemplatePlugin<Options> = Omit<NuxtPlugin, 'src'> & NuxtTemplate<Options>;
...
declare type DeepPartial<T> = T extends Function ? T : T extends Record<string, any> ? {
...
declare type NuxtConfigLayer = ConfigLayer<NuxtConfig & {
...
declare const _default: {
    messages: untyped.SchemaDefinition;
    render: untyped.SchemaDefinition;
    router: untyped.SchemaDefinition;
    server: untyped.SchemaDefinition;
    cli: untyped.SchemaDefinition;
    generate: untyped.SchemaDefinition;
};

export { AppConfig, AppConfigInput, Component, ComponentsDir, ComponentsOptions, ImportPresetWithDeprecation, ImportsOptions, MetaObject, ModuleContainer, ModuleDefinition, ModuleMeta, ModuleOptions, Nuxt, NuxtApp, NuxtAppConfig, NuxtCompatibility, NuxtCompatibilityIssue, NuxtCompatibilityIssues, NuxtConfig, NuxtConfigLayer, _default as NuxtConfigSchema, NuxtHookName, NuxtHooks, NuxtLayout, NuxtMiddleware, NuxtModule, NuxtOptions, NuxtPage, NuxtPlugin, NuxtPluginTemplate, NuxtTemplate, PrivateRuntimeConfig, PublicRuntimeConfig, ResolvedNuxtTemplate, RouterConfig, RouterConfigOptions, RouterConfigSerializable, RouterOptions, RuntimeConfig, ScanDir, TSReference, ViteConfig };
