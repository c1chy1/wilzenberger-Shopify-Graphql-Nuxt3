...
import { useNuxt, resolveFiles, defineNuxtModule, addPlugin, addVitePlugin, addWebpackPlugin, addTemplate, findPath, isIgnored, resolveAlias, addPluginTemplate, logger, resolvePath, nuxtCtx, addComponent, tryResolveModule, installModule, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils, importModule } from '@nuxt/kit';
import escapeRE from 'escape-string-regexp';
import fse from 'fs-extra';
import { encodePath, parseURL, parseQuery, withQuery, joinURL, withTrailingSlash, withoutLeadingSlash } from 'ufo';
...
const TreeShakeTemplatePlugin = createUnplugin((options) => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      const s = new MagicString(code);
      s.replace(COMPONENTS_RE, (r) => r.replace(/<([^>]*[^/])\/?>[\s\S]*$/, (chunk, el) => {
...
function addDeclarationTemplates(ctx, options) {
  ...
  addTemplate({
    ...
    getContents: () => "// Generated by auto imports\n" + (options.autoImport ? ctx.generateTypeDeclarations({ resolvePath: r }) : "// Implicit auto importing is disabled, you can use explicitly import from `#imports` instead.")
  });
}
...
async function initNitro(nuxt) {
  ...
  const nitroConfig = defu(_nitroConfig, {
    ...
    prerender: {
      ...
      routes: [].concat(nuxt.options._generate ? ["/", "/200.html", ...nuxt.options.generate.routes] : []).concat(nuxt.options.ssr === false ? ["/index.html", "/200.html", "/404.html"] : [])
    },
  ...
  if (nuxt.options.dev) {
    ...
    nuxt.hook("server:devMiddleware", (m) => {
      devMiddlewareHandler.set(toEventHandler(m));
    });
