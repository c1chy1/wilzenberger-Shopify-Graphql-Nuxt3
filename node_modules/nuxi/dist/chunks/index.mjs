import require$$0$2, { createServer as createServer$2 } from 'http';
import require$$1, { createServer as createServer$1 } from 'https';
import { promisify } from 'util';
import require$$0, { promises, existsSync, writeFileSync, chmodSync, readFileSync, constants, statSync } from 'fs';
import os, { networkInterfaces } from 'os';
import { g as gray, c as cyan, u as underline, b as bold } from '../shared/nuxi.3d7f34cc.mjs';
import { createServer } from 'net';
import require$$0$1, { join } from 'path';
import { d as defu } from '../shared/nuxi.e150c956.mjs';
...
const baseOpen = async (options) => {
  ...
  const subprocess = require$$0$3.spawn(command, cliArguments, childProcessOptions);
  if (options.wait) {
...
async function listen(handle, opts = {}) {
  ...
  let server;
  let addr;
  const getURL = (host, baseURL) => `${addr.proto}://${host || opts.hostname || addr.addr}:${addr.port}${baseURL || opts.baseURL}`;
  let https = false;
  if (opts.https) {
  ...
  if (opts.clipboard) {
    const clipboardy = await import('./index2.mjs').then((r) => r.default || r);
    await clipboardy.write(getURL("localhost")).catch(() => {
      opts.clipboard = false;
    });
  }
...
async function resolveCert(opts, host) {
  if (opts.key && opts.cert) {
    ...
    const r = (s) => isInline(s) ? s : promises.readFile(s, "utf-8");
    return {
