...
import { c as commonjsGlobal$1 } from '../shared/nuxi.267db104.mjs';
...
import { y as yallist } from '../shared/nuxi.f580be36.mjs';
import require$$2$1 from 'util';
import require$$12 from 'crypto';
import { r as resolve$1, a as relative, d as dirname$3 } from '../shared/nuxi.dae0d833.mjs';
...
const pipeline = promisify$1(Stream$1.pipeline);
...
async function download(url, filePath, opts = {}) {
  ...
  await promisify$1(pipeline$1)(res.body, stream);
...
async function downloadTemplate(input, opts = {}) {
  ...
  const cwd = resolve$1(opts.cwd || ".");
  const extractPath = resolve$1(cwd, opts.dir || template.defaultDir);
  if (opts.forceClean) {
    await rm(extractPath, { recursive: true, force: true });
  }
  if (!opts.force && existsSync(extractPath) && readdirSync$1(extractPath).length) {
    throw new Error(`Destination ${extractPath} already exists.`);
  }
  await mkdir$2(extractPath, { recursive: true });
  const tmpDir = resolve$1(homedir(), ".giget", opts.provider, template.name);
  const tarPath = resolve$1(tmpDir, (template.version || template.name) + ".tar.gz");
...
export { FormData as F, File as a, init$1 as i };
