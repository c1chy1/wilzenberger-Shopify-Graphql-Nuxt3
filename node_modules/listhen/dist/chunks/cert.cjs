...
const forge__default = /*#__PURE__*/_interopDefaultLegacy(forge);
const ipRegex__default = /*#__PURE__*/_interopDefaultLegacy(ipRegex);
...
async function generateSSLCert(opts) {
  ...
  const extensions = [
    ...
    { name: "extKeyUsage", serverAuth: true, clientAuth: true },
    {
      ...
      altNames: opts.domains.map((domain) => {
        ...
        const isIp = ipRegex__default({ exact: true }).test(domain);
        if (isIp) {
  ...
  const ca = forge__default.pki.certificateFromPem(opts.caCert);
...
async function generateCert(opts) {
  ...
  const generateKeyPair = node_util.promisify(forge__default.pki.rsa.generateKeyPair.bind(forge__default.pki.rsa));
  ...
  const signWith = opts.signWith ? forge__default.pki.privateKeyFromPem(opts.signWith) : keyPair.privateKey;
